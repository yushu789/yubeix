...
| sharedTopBarEnabled     | Boolean                                     | false         | Draws the pair's chrome bars once, fixed at the top of the window, during a transition (the iOS navigation-bar behavior) instead of sliding with their scenes. Requires each scene to use the `ScreenScaffold` chrome bar; scenes without one are unaffected |
