...
/**
 * Yubeix default [Indication] that draws a rectangular overlay when pressed.
 */
@Immutable
class YubeixIndication(
    private val color: Color = Color.Black,
) : IndicationNodeFactory {
    override fun create(interactionSource: InteractionSource): DelegatableNode = YubeixIndicationInstance(interactionSource, color)

    override fun hashCode(): Int = color.hashCode()

    override fun equals(other: Any?): Boolean {
        if (this === other) return true
        if (other !is YubeixIndication) return false
        if (color != other.color) return false
        return true
    }

    private class YubeixIndicationInstance(
